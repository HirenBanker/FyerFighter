...
def backtest_strategy(ticker, start_date, end_date, fyers, stoploss=5, target=10, initial_capital=10000, interval='1d'):
    ...
    ha_data['EMA_Lows'] = ema_lows(ha_data, period=5)
    ha_data['EMA_Highs'] = ema_highs(ha_data, period=5)
    
    # Calculate TSI and TSI signal
    ha_data['TSI'], ha_data['TSI_Signal'] = calculate_tsi(ha_data['HA_Close'], r=30, s=13, signal_period=13)
    ...
    for i in range(1, len(ha_data)):
        ...
        # Entry Condition - Always enter if no position
        if position == 0:
            entry_price = price
            quantity = capital/price  # Fixed quantity
...
def should_enter_trade(fyers, ticker, live_price, interval="1m"):
    # Always return True to take trade immediately
    return True
