...
from instantbuy.i_strategy import backtest_strategy, should_enter_trade, should_exit_trade

def initialize_trading_state():
    if 'instantbuy_position' not in st.session_state:
        st.session_state.instantbuy_position = 0
    if 'instantbuy_entry_price' not in st.session_state:
        st.session_state.instantbuy_entry_price = None
    if 'instantbuy_entry_time' not in st.session_state:
        st.session_state.instantbuy_entry_time = None
    if 'instantbuy_trade_log' not in st.session_state:
        st.session_state.instantbuy_trade_log = []
    if 'instantbuy_running' not in st.session_state:
        st.session_state.instantbuy_running = False
...
def get_condition_values(fyers, ticker, live_price, interval, stoploss, target):
...
def show_trade():
    st.subheader("Instant Buy Strategy - Live Trading")
    ...
    with col1:
        ...
        symbol = st.text_input("Ticker Symbol", value="NSE:SBIN-EQ", key="ib_symbol")
        interval = st.selectbox("Interval", ["1m", "2m", "3m", "5m", "15m", "30m", "1h", "1d"], key="ib_interval")
        stoploss = st.number_input("Stoploss (%)", min_value=0.1, value=5.0, key="ib_stoploss")
        target = st.number_input("Target (%)", min_value=0.1, value=10.0, key="ib_target")
        qty = st.number_input("Quantity", min_value=1, value=1, step=1, key="ib_qty")
        trading_mode = st.radio("Trading Mode", ["Paper Trade", "Live Trade"], key="ib_mode")
    ...
    if st.session_state.instantbuy_running:
        st.session_state.instantbuy_running = True
        
        if st.button("Stop Trading", key="ib_stop"):
            st.session_state.instantbuy_running = False
            if st.session_state.instantbuy_position == 1:
                price = get_latest_price(st.session_state.fyers_client, symbol.upper())
                if price:
                    dt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    pnl = (price - st.session_state.instantbuy_entry_price) * st.session_state.instantbuy_qty
                    st.session_state.instantbuy_trade_log.append([dt, "SELL", price, st.session_state.instantbuy_qty, round(pnl, 2)])
                    st.session_state.instantbuy_position = 0
            export_trades(symbol.upper(), st.session_state.instantbuy_trade_log)
            st.rerun()
        ...
        while st.session_state.instantbuy_running and update_count < max_updates:
            try:
                price = get_latest_price(st.session_state.fyers_client, symbol.upper())
                ...
                if st.session_state.instantbuy_position == 0:
                    if should_enter_trade(st.session_state.fyers_client, symbol.upper(), price, interval):
                        st.session_state.instantbuy_entry_price = price
                        st.session_state.instantbuy_entry_time = now
                        st.session_state.instantbuy_qty = qty
                        st.session_state.instantbuy_position = 1
                        st.session_state.instantbuy_trade_log.append([dt, "BUY", price, qty, 0.0])
                        export_trades(symbol.upper(), st.session_state.instantbuy_trade_log)
                        status_placeholder.success(f"BUY executed at ₹{price:.2f}")
                        entry_placeholder.metric("Entry Price", f"₹{st.session_state.instantbuy_entry_price:.2f}", delta=f"@ {st.session_state.instantbuy_entry_time}")
                    else:
                        status_placeholder.info("Waiting for entry signal...")
                else:
                    if should_exit_trade(st.session_state.fyers_client, symbol.upper(), st.session_state.instantbuy_entry_price, stoploss, target, price, interval):
                        pnl = (price - st.session_state.instantbuy_entry_price) * qty
                        st.session_state.instantbuy_trade_log.append([dt, "SELL", price, qty, round(pnl, 2)])
                        export_trades(symbol.upper(), st.session_state.instantbuy_trade_log)
                        pnl_placeholder.metric("P&L", f"₹{pnl:.2f}", delta=f"{((pnl / (st.session_state.instantbuy_entry_price * qty)) * 100):.2f}%")
                        ...
                        entry_placeholder.metric("Entry Price", f"₹{st.session_state.instantbuy_entry_price:.2f}", delta=f"@ {st.session_state.instantbuy_entry_time}")
                        if st.session_state.instantbuy_position == 1:
                            current_pnl = (price - st.session_state.instantbuy_entry_price) * qty
                            pnl_placeholder.metric("Current P&L", f"₹{current_pnl:.2f}", delta=f"{((current_pnl / (st.session_state.instantbuy_entry_price * qty)) * 100):.2f}%")
                ...
                with conditions_placeholder.container():
                    ...
                    with cond_col2:
                        st.write(f"SL Price: ₹{st.session_state.instantbuy_entry_price * (1 - stoploss / 100):.2f}" if st.session_state.instantbuy_position == 1 else f"SL: {stoploss}%")
                        st.write(f"Target Price: ₹{st.session_state.instantbuy_entry_price * (1 + target / 100):.2f}" if st.session_state.instantbuy_position == 1 else f"Target: {target}%")
        ...
        if st.button("Start Trading", key="ib_start"):
            st.session_state.instantbuy_running = True
            st.rerun()

def show_backtest():
    st.subheader("Instant Buy Backtest")
    ...
    with col1:
        symbol = st.text_input("Ticker Symbol", value="NSE:SBIN-EQ", key="ib_bt_symbol")
        start_date = st.date_input("Start Date", value=datetime.now() - timedelta(days=365), key="ib_bt_start")
        end_date = st.date_input("End Date", value=datetime.now(), key="ib_bt_end")
        interval = st.selectbox("Interval", ["1m", "5m", "15m", "1h", "1d"], key="ib_bt_interval")
    
    with col2:
        stoploss = st.number_input("Stoploss (%)", min_value=0.1, value=5.0, key="ib_bt_stoploss")
        target = st.number_input("Target (%)", min_value=0.1, value=10.0, key="ib_bt_target")
        initial_capital = st.number_input("Initial Capital", min_value=1000, value=10000, step=1000, key="ib_bt_capital")
    
    if st.button("Run Backtest", key="ib_run_backtest"):
